...
Being the first release of the pipeline (v.1), and despite accurate testing, it may contain bugs. In case of problems, please get in touch with us.  
...
## Overview
SmithHunter setup entails cloning the latest distribution from GitHub and running the setup.sh executable. Two dependencies are needed and not included in the distribution: conda and PITA. At first execution, setup.sh will search the local system for conda and PITA and, if not available, will offer the opportunity to download/install both.
## Procedure
- Create a folder for SmithHunter in your system and cd. 
> cd ~
> mkdir SmithHunter
> cd SmithHunter

- Git-clone the SmithHunter repository using the following command:
> git clone XXXX.....
...
- The installer will look for a conda installation on the local system. If not available will offer to install miniconda3 from its website (https://docs.anaconda.com/free/miniconda/index.html). Upon a positive answer, will install miniconda3 in the user's home directory.
- The installer will look for PITA on the local system. If not available will offer to install PITA from GitHub (https://github.com/elenacorni/pita). Upon a positive answer, will install PITA in the SmithHunter folder.
...
- One more key information is the **working directory**. All input (apart from raw sequence reads) and output files from an analysis have to, and will be written, in be in the working directory. This is, by default, the directory from which the script is run, but can be modified using the -W option (see below).
...
The first module, named smithHunterA.sh, focusses on the **identification and filtering of presumptive smithRNA** sequences, defined as centroids of clusters with significant transcription levels and a narrow 5’ transcription boundary. It takes as input one or more small RNA libraries (replicates), the sequence of the mitochondrial genome and (optionally) the sequence of the nuclear genome of the species of interest. Main output is a list of presumptive smithRNA sequences, filtered based on parameters defined by the user, as well as graphics depicting: a) read coverage over the mitochondrial genome, global and per preplica; b) cluster position/abundance on the mitochondrial genome and c) 5’ and 3’ end conservation.
...
### Command line
    ...
    bash -i smithHunterA.sh -O organism_ID -W ~/example/ [options]
...
~/example/RUDI_mito.fna (the mitochondrial genome)
~/example/rawdata/MicroReplicate1_1.fastq.gz (smallRNA sequencing data)
~/example/rawdata/MicroReplicate2_1.fastq.gz
~/example/rawdata/MicroReplicate3_1.fastq.gz
~/prog/SmithHunter_v0/ (SmithHunter installation folder)

Command:

    bash -i ~/prog/SmithHunter_v0/smithHunterA.sh -O RUDI \
    -W ~/example/ \
    -F ~/example/rawdata/ \
    -T SE \
...
~/example/RUDI_nuc.fna (the nuclear genome)
~/example/RUDI_mito.fna (the mitochondrial genome)
~/example/rawdata/MicroReplicate1_1.fastq.gz (smallRNA sequencing data)
~/example/rawdata/MicroReplicate1_2.fastq.gz
~/example/rawdata/MicroReplicate2_1.fastq.gz
~/example/rawdata/MicroReplicate2_2.fastq.gz
~/example/rawdata/MicroReplicate3_1.fastq.gz
~/example/rawdata/MicroReplicate3_2.fastq.gz
~/prog/SmithHunter_v0/ (SmithHunter installation folder)
...
The user can **manually edit the presumptive smithRNA list** produced by module A, adding or removing presumptive smithRNAs before running module B. More specifically, we envision that the user may manually select a) smithRNAs with more conserved 3’ and 5’ end (based on output Plot.html from module A); b) smithRNAs from specific parts of the genome; c) smithRNAs within a specific size range.

The file to be edited is `/7_organismID_smithRNAs/presumptive_smithRNAs.fa` within the working directory (i.e. the corresponding file in the `organismID_main_outputs` folder is for review only and does not enter the module B script).

An additional script is distributed with SmithHunter to help perform this task (`sharp_smith.py`). The scoring scheme is still experimental and we do not advise a non supervised use of the script to filter presumptive smithRNAs at this step. Nevertheless it may be used for a quick filtering of very low scoring smithRNAs or in conjunction with manual revision of end conservation in `Plot.html` (from module A). The script has to be executed from the smithHunterA conda environment.
...
### Command line:
    ...
    bash -i smithHunterB.sh -O organism_ID -W ~/example/ -P ~/path/to/pita/folder/ [options]
...
Full path to the folder where the PITA executable is. Use an absolute, not relative, path. No default.
...
We have already run module A, specifying ‘RUDI’ as organism_ID and `~/example/` as working directory. PITA (pita_prediction.pl) is in folder `~/programs/SmithHunter_v0/pita/pita_cpp/`. We have the transcriptome sequences in fasta format and an annotation of 5’ and 3’ UTRs in bed format. We want to use conservative sets for target identification, we will use defaults. 

Files are organized as follows:

~/prog/SmithHunter_v0/ (SmithHunter installation folder)
~/programs/SmithHunter_v0/pita/pita_cpp/pita_prediction.pl (pita executable)
~/example/RUDI_Transcripts.fasta (the transcriptome)
~/example/RUDI_UTR.bed (5’ and 3’ UTRs annotation)
~/example/7_RUDI_smithRNAs/presumptive_smithRNAs.fa  (list of presumptive smithRNAs from module A)

Command:

    bash -i ~/prog/SmithHunter_v0/smithHunterB.sh -O RUDI \
    -W ~/example/ \
    -P ~/programs/SmithHunter_v0/pita/pita_cpp/


### Example 2

We have already run module A, specifying ‘RUDI’ as organism_ID and `~/example/` as working directory. PITA (pita_prediction.pl) is in folder  `~/programs/SmithHunter_v0/pita/pita_cpp/`. We have the transcriptome sequence in fasta format and an annotation of 5’ and 3’ UTRs in bed format.
...
~/programs/SmithHunter_v0/ (SmithHunter script folder)
~/programs/SmithHunter_v0/pita/pita_cpp/pita_prediction.pl (pita executable)
~/example/RUDI_Transcripts.fasta (the transcriptome)
~/example/RUDI_UTR.bed (5’ and 3’ UTRs annotation)
~/example/7_RUDI_smithRNAs/presumptive_smithRNAs.fa  (list of presumptive smithRNAs from module A)
