...
# difference from red and blue are peaks that were removed as of nuclear origin (i.e. reads can remap on nuclear geome)
plot(prima$base, prima$tot, type='l', xlab='position over genome', ylab='coverage (all replicates)', col='#DC3220')
points(dopo$base, dopo$tot, type='l', col='#005AB5')
abline(h=T1, col='black')

#metto la legenda
legend("topright", legend = c("Mito reads", "Mito-unique reads"), col = c("#DC3220", "#005AB5"), lty = 1, cex = 0.8)
...
plot(dopo$base, dopo$tot, type='l', xlab='position over genome', ylab='coverage (all replicates, detail)', col='#005AB5', ylim=c(0, 5*T1))
abline(h=T1, col='black')
...
for (r in 1:n_replicates) {
    ...
    plot(dopo$base, dopo[,r+1], type='l', xlab='position over genome', ylab=paste('coverage (', rname, '), detail', sep=''), col='#005AB5', ylim=c(0, 5*T2_replica))
    abline(h=T2_replica, col='black')
}
trash <- dev.off()
...
for (campo in campi) { # for each of the four plotting fields
    ...
    for (c in 1:nrow(candidates)) { # for each candidate
        if (candidates[c,3] == '+'){ # in red if on plus strand (genome as submitted)
            rect(candidates[c,1], 0, candidates[c,2], candidates[c,4], border='#DC3220')
        }
        if (candidates[c,3] == '-'){ # in blue if on minus strand
            rect(candidates[c,1], 0, candidates[c,2], candidates[c,4], border='#005AB5')
        }
    }
